...
export const nn_module_metadata: Record<string, ModuleMetadata> = {
    ...
    "RNN": {
        ...
        code_generator: (params) => `nn.RNN(input_size=${params['input_size']}, hidden_size=${params['hidden_size']}, ` +
            `num_layers=${params['num_layers'] ?? 1}, nonlinearity='${params['nonlinearity'] ?? 'tanh'}', ` +
            `bias=${params['bias'] ?? true}, batch_first=${params['batch_first'] ?? false}, ` +
            `dropout=${params['dropout'] ?? 0}, bidirectional=${params['bidirectional'] ?? false})`
    },

    "LSTM": {
        required_params: ["input_size", "hidden_size"],
        optional_params: ["num_layers", "bias", "batch_first", "dropout", "bidirectional"],
        code_generator: (params) => `nn.LSTM(input_size=${params['input_size']}, hidden_size=${params['hidden_size']}, ` +
            `num_layers=${params['num_layers'] ?? 1}, bias=${params['bias'] ?? true}, ` +
            `batch_first=${params['batch_first'] ?? false}, dropout=${params['dropout'] ?? 0}, ` +
    ...
    "GRU": {
        required_params: ["input_size", "hidden_size"],
        optional_params: ["num_layers", "bias", "batch_first", "dropout", "bidirectional"],
        code_generator: (params) => `nn.GRU(input_size=${params['input_size']}, hidden_size=${params['hidden_size']}, ` +
            `num_layers=${params['num_layers'] ?? 1}, bias=${params['bias'] ?? true}, ` +
            `batch_first=${params['batch_first'] ?? false}, dropout=${params['dropout'] ?? 0}, ` +
            `bidirectional=${params['bidirectional'] ?? false})`
    },
    ...
    "TransformerEncoderLayer": {
        ...
        code_generator: (params) => `nn.TransformerEncoderLayer(d_model=${params['d_model']}, nhead=${params['nhead']}, ` +
            `dim_feedforward=${params['dim_feedforward'] ?? 2048}, dropout=${params['dropout'] ?? 0.1}, ` +
            `activation='${params['activation'] ?? 'relu'}', batch_first=${params['batch_first'] ?? false})`
    },
    ...
    "Embedding": {
        ...
        code_generator: (params) => `nn.Embedding(num_embeddings=${params['num_embeddings']}, embedding_dim=${params['embedding_dim']}, ` +
            `padding_idx=${params['padding_idx'] ?? 'None'}, max_norm=${params['max_norm'] ?? 'None'}, ` +
