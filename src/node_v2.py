...
class Node(abc.ABC):
    ...
    def set_params(self, params: dict) -> 'Node':
        """
        ...
        try:
            ...
            raise InvalidLayerParametersError(f"Parameter validation failed: {str(e)}")
        self._update_out_shape()
        return self

    def connect_to(self, target: 'Node') -> None:
        """
        Connect this node's output to the target's input.
        """
        if target.in_shape is not None:
            if not Node.shapes_equal(self.out_shape, target.in_shape):
                raise InShapeMismatchError(
                    f"Input shape mismatch: cannot connect output shape {self.out_shape} to input shape {target.in_shape}"
                )
        else:
            target.set_in_shape(self.out_shape)
        
        self.out_node = target
        target.in_node = self

    def set_in_shape(self, shape: Shape) -> 'Node':
        """
        Set the input shape of the node.
        """
        self.in_shape = shape
        self._update_out_shape()
        return self
    ...
    def compute_out_shape(self, in_shape: Shape) -> Shape:
        ...
        In some cases, the layer parameters already defined the out_shape, like Reshape 
